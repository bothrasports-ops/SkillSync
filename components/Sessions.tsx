      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col md:flex-row gap-6 items-start md:items-center shadow-sm">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            {session.status === SessionStatus.ACCEPTED && (
                <>
                    {!isProvider ? (
                        <button onClick={() => setRatingModal(session.id)} className="flex-1 md:flex-none px-6 py-2 bg-emerald-600 text-white rounded-xl text-sm font-bold hover:bg-emerald-700 transition shadow-lg shadow-emerald-50">Rate & Complete</button>
                    ) : (
                        <span className="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-sm font-bold border border-indigo-100">Teaching Active</span>
                    )}
                </>
            )}
