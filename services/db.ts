...
export const db = {
  ...
  async init(): Promise<AppState> {
    ...
    const { data: profiles } = pRes;
    const { data: sessions } = sRes;
    const { data: invitations } = iRes;
  ...
  async verifyAndSignIn(contact: string): Promise<User> {
    // 1. Check for invitation
    const { data: invite, error: iErr } = await supabase
        .from('invitations')
        .select('*')
        .eq('email_or_phone', contact)
        .eq('status', 'pending')
        .single();

    if (iErr || !invite) {
        // Check if user is already an admin/existing user (bypassing invite check)
    ...
    // 3. Create new profile (First time)
